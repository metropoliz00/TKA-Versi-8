...
import { Users, FileText, Download, Upload, Loader2, Plus, Search, Edit, Trash2, X, Camera, Save, Settings } from 'lucide-react';
...
const DaftarPesertaTab = ({ currentUser, onDataChange }: { currentUser: User, onDataChange: () => void }) => {
    ...
    const handleDelete = async (username: string) => { 
        const confirmed = await showAlert("Yakin ingin menghapus pengguna ini?", { type: 'confirm' });
        if(!confirmed) return; 
        setLoading(true); 
        try { 
            ...
            await showAlert("Gagal menghapus user.", { type: 'error' }); 
        } finally { 
            setLoading(false); 
        } 
    };
    ...
    const handleAdd = () => { 
        setFormData({ 
            id: '', username: '', password: '', nama_lengkap: '', role: 'siswa', 
            kelas_id: currentUser.role === 'admin_sekolah' ? currentUser.kelas_id : '', 
    ...
    const handleSave = async (e: React.FormEvent) => { 
        ...
        try { 
            ...
            // Enforce restrictions for non-admin_pusat
            if (currentUser.role !== 'admin_pusat') {
                payload.role = 'siswa';
                payload.kelas_id = currentUser.kelas_id || payload.kelas_id;
                payload.school = currentUser.kelas_id || payload.school;
                payload.kecamatan = currentUser.kecamatan || payload.kecamatan;
    ...
    const uniqueKecamatans = useMemo<string[]>(() => {
        ...
        if (currentUser.role === 'proktor' && currentUser.id_sekolah) {
            relevantUsers = users.filter(u => u.id_sekolah === currentUser.id_sekolah);
        } else if (currentUser.role === 'admin_kecamatan' && currentUser.id_kecamatan) {
            relevantUsers = users.filter(u => u.id_kecamatan === currentUser.id_kecamatan);
        }
        const kecamatans = new Set(relevantUsers.map(u => u.kecamatan).filter(Boolean));
    ...
    }, [users, filterKecamatan, currentUser]);
    const filteredUsers = useMemo(() => {
        let filtered = users.filter(u => u.role === 'siswa'); // Always filter for siswa role

        // Apply role-based filtering first
        if (currentUser.role === 'proktor' && currentUser.id_sekolah) {
            filtered = filtered.filter(u => u.id_sekolah === currentUser.id_sekolah);
        } else if (currentUser.role === 'admin_kecamatan' && currentUser.id_kecamatan) {
    ...
    reader.onload = async (evt) => {
        try {
            ...
            for (let i = 1; i < data.length; i++) {
                ...
                let school = String(row[5] || '');
                let kecamatan = String(row[6] || '');
                let id_sekolah = String(row[8] || '');
                let id_gugus = String(row[9] || '');
                let id_kecamatan = String(row[10] || '');

                // Enforce restrictions for non-admin_pusat
                if (currentUser.role !== 'admin_pusat') {
                    role = 'siswa'; // Only allow uploading students
                    school = currentUser.kelas_id || school;
                    kecamatan = currentUser.kecamatan || kecamatan;
                    id_sekolah = currentUser.id_sekolah || id_sekolah;
                    id_gugus = currentUser.id_gugus || id_gugus;
                    id_kecamatan = currentUser.id_kecamatan || id_kecamatan;
                }
            ...
            if (parsedUsers.length > 0) {
                ...
                let successMessage = `Berhasil mengimpor ${parsedUsers.length} pengguna.`;
    ...
    const downloadTemplate = () => { const ws = XLSX.utils.json_to_sheet([ { "Username": "siswa001", "Password": "123", "Role (siswa/admin_sekolah/admin_pusat)": "siswa", "Nama Lengkap": "Ahmad Siswa", "L/P": "L", "Sekolah / Kelas": "UPT SD Negeri Remen 2", "Kecamatan": "Jenu", "Link Foto (Opsional)": "https://...", "ID Sekolah": "SCH001", "ID Gugus": "G01", "ID Kecamatan": "KEC01" }, { "Username": "proktor01", "Password": "123", "Role (siswa/admin_sekolah/admin_pusat)": "admin_sekolah", "Nama Lengkap": "Pak Guru", "L/P": "L", "Sekolah / Kelas": "UPT SD Negeri Glodog", "Kecamatan": "Palang", "Link Foto (Opsional)": "", "ID Sekolah": "SCH002", "ID Gugus": "G02", "ID Kecamatan": "KEC02" } ]); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "Template_User"); XLSX.writeFile(wb, "Template_Import_User.xlsx"); };
                ...
                {currentUser.role === 'admin_pusat' && (
                    ...
                    <select 
                        ...
                        onChange={e => {
                            ...
                            setFilterSchool(val);
                            if (val !== 'all') {
                                const schoolUser = users.find(u => u.kelas_id === val);
                     ...
                     <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden max-h-[90vh] flex flex-col">
                         ...
                         <div className="p-6 overflow-y-auto">
                            <form onSubmit={handleSave} className="space-y-4">
                                ...
                                <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Nama Lengkap</label><input required type="text" className="w-full p-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-100 outline-none" value={formData.nama_lengkap} onChange={e => setFormData({...formData, nama_lengkap: e.target.value})} /></div>
                                <div className="grid grid-cols-2 gap-4"><div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Role</label><select className="w-full p-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-100 outline-none bg-white" value={formData.role} onChange={e => setFormData({...formData, role: e.target.value as User['role']})} disabled={currentUser.role !== 'admin_pusat'}><option value="siswa">Siswa</option><option value="admin_sekolah">Proktor</option><option value="admin_pusat">Admin Pusat</option></select></div><div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Jenis Kelamin</label><select className="w-full p-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-100 outline-none bg-white" value={formData.jenis_kelamin} onChange={e => setFormData({...formData, jenis_kelamin: e.target.value})}><option value="L">Laki-laki</option><option value="P">Perempuan</option></select></div></div>
                                 {(formData.role === 'siswa' || formData.role === 'admin_sekolah') && (
                                    <>
                                        <div className="grid grid-cols-2 gap-4">
                                            <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">{formData.role === 'siswa' ? 'Kelas / Sekolah' : 'Nama Sekolah (Untuk Proktor)'}</label><input required type="text" className="w-full p-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-100 outline-none" value={formData.kelas_id} onChange={e => setFormData({...formData, kelas_id: e.target.value})} placeholder={formData.role === 'siswa' ? "Sekolah" : "Sekolah"} disabled={currentUser.role !== 'admin_pusat'} /></div>
                                            <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Kecamatan</label><input type="text" className="w-full p-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-100 outline-none" value={formData.kecamatan} onChange={e => setFormData({...formData, kecamatan: e.target.value})} placeholder="Kecamatan" disabled={currentUser.role !== 'admin_pusat'} /></div>
                                        </div>
                                        <div className="grid grid-cols-2 gap-4">
